...
const yourAvailableWidth = 300;
const yourCellWidth = 100; 
...
<template>
  <table cellspacing="0">
    ...
    <tbody v-for="emailGroup in emailGroupsByDate" :key="emailGroup[0].datetime">
      <tr v-for="{ from, to: recipients, subject, datetime } in emailGroup" :key="subject + datetime">
        <td>{{ from }}</td>
        <td>
          <RecipientsDisplay :recipients="recipients" :availableWidth="yourAvailableWidth" :cellWidth="yourCellWidth" />
        </td>
        <td>{{ subject }}</td>
